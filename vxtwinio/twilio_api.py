...
from vxtwinio.responses import (
...
class Applications(ListResponse):
    """ Used for responding with a list of Applications for the
...
class TwilioAPIServer(object):
    ...
    def usage_exception(self, request, failure):
        ...
        return self._format_response(
            request, ErrorResponse.from_exception(failure.value),
    ...
    @app.route(
        '/Accounts/<string:account_sid>/IncomingPhoneNumbers',
        defaults={'format_': ''}, methods=['GET'])
    @app.route(
        '/Accounts/<string:account_sid>/IncomingPhoneNumbers<string:format_>',
        methods=['GET'])
    def get_phone_numbers(self, request, account_sid, format_):
        phone_numbers = IncomingPhoneNumbers(request.uri, [])
        return self._format_response(request, phone_numbers, format_)
